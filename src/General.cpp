...
Model General::getCrossModel(float x, float y, float z, TextureFace face, int yOffset, bool flat)
{
	...
	float flHeight = flat ? y + yOffset : y + yOffset;
	float frHeight = flat ? flHeight : y + yOffset;
	float blHeight = flat ? flHeight : y + yOffset;
	float brHeight = flat ? flHeight : y + yOffset;
